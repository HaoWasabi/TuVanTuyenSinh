...
function Get-VersionKey {
    ...
    if ($match.Success) {
        try { return [version]$match.Value } catch { return [version]'0.0.0.0' }
    }
...
function Add-LatestJar {
    ...
    if ($Required) { throw "Thieu dependency bat buoc: $JarPrefix" }
}

if (-not (Test-Path $sourceDir)) { throw "Khong tim thay thu muc nguon: $sourceDir" }
if (-not (Test-Path $repo)) { throw "Khong tim thay kho .m2: $repo" }

$javaFiles = Get-ChildItem $sourceDir -Recurse -Filter '*.java' | Select-Object -ExpandProperty FullName
if (-not $javaFiles -or $javaFiles.Count -eq 0) { throw 'Khong tim thay file .java de bien dich' }

# Làm sạch thư mục target trước khi build mới
if (Test-Path $outputDir) { Remove-Item -Recurse -Force $outputDir | Out-Null }
New-Item -ItemType Directory -Force $outputDir | Out-Null

# Quét tất cả các file jar trong .m2 để làm classpath compile
...
javac "@$argsFile"
if ($LASTEXITCODE -ne 0) { throw "Compile that bai (exit code: $LASTEXITCODE)" }

# Sao chép tài nguyên tĩnh và các file HTML Thymeleaf sang thư mục đích
if (Test-Path $resourceDir) {
    Write-Host "Copying web resources (Thymeleaf templates & static files)..." -ForegroundColor Cyan
    Copy-Item -Path "$resourceDir\*" -Destination $outputDir -Recurse -Force
}

Write-Host "Compile va dong goi tai nguyen Web thanh cong. Output: $outputDir" -ForegroundColor Green