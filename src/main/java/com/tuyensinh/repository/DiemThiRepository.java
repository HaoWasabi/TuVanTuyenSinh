...
public class DiemThiRepository {
    ...
    public Optional<DiemThi> findById(Integer idnv) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            DiemThi result = session.createQuery(
                    "from DiemThi d where d.idnv = :idnv and d.status = 'ACTIVE'", DiemThi.class)
    ...
    public List<Object[]> thongKeSoLuongTheoPhuongThuc() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            String hql = "SELECT d.dPhuongthuc, COUNT(d) FROM DiemThi WHERE d.status = 'active' GROUP BY d.dPhuongthuc";
    ...
    public Object[] thongKeDiemTrungBinhTatCaMon() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            String hql = "SELECT AVG(d.toan), AVG(d.vatLi), AVG(d.hoaHoc), AVG(d.sinhHoc), " +
                        "AVG(d.lichSu), AVG(d.diaLi), AVG(d.nguVan), AVG(d.tinHoc), AVG(d.ktpl) FROM DiemThi d and d.status = 'ACTIVE'";
            return session.createQuery(hql, Object[].class).uniqueResult();
        }
    }
