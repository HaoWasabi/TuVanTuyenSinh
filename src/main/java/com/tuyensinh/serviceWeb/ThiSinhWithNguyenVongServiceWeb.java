...
public class ThiSinhWithNguyenVongServiceWeb {
    ...
    /**
     * Lấy thông tin thí sinh + danh sách nguyên vọng xét tuyển theo CCCD và ngày
     * sinh
     * 
     * @param cccd     Số CCCD của thí sinh
     * @param ngaySinh Ngày sinh (định dạng: ddMMyyyy hoặc dd/MM/yyyy)
     * @return Optional chứa ThiSinhWithNguyenVongResponse hoặc empty nếu không tìm
     *         thấy
     */
    public Optional<ThiSinhWithNguyenVongResponse> getThiSinhWithNguyenVong(String cccd, String ngaySinh) {
        ...
        // Nếu không tìm được theo lookup trực tiếp, cố gắng tìm linh hoạt hơn (ví dụ DB
        // chứa mã TS_xxx hoặc định dạng khác)
        if (thiSinhOpt.isEmpty()) {
            // Tìm trong tất cả thí sinh, so sánh linh hoạt CCCD và ngày sinh
            thiSinhOpt = thiSinhRepository.findAll().stream()
                    .filter(ts -> cccdMatches(ts.getCccd(), cccd) && dateMatches(ts.getNgaySinh(), ngaySinh))
                    .findFirst();
        }
        ...
        // Bước 2: Lấy danh sách nguyên vọng theo CCCD (dùng CCCD thực tế từ DB, không
        // phải input form)
        List<NguyenVong> nguyenVongList = nguyenVongRepository.findByCccd(thiSinh.getCccd());

        // Bước 3: Chuyển đổi sang DTO response
        return Optional.of(buildThiSinhWithNguyenVongResponse(thiSinh, nguyenVongList));
    }

    // Flexible CCCD comparison (accept exact, case-insensitive, or digits-only
    // match)
    private boolean cccdMatches(String stored, String input) {
        ...
        if (stored.equalsIgnoreCase(input))
            return true;
        String sDigits = stored.replaceAll("\\D", "");
        String iDigits = input.replaceAll("\\D", "");
        if (!sDigits.isEmpty() && !iDigits.isEmpty() && sDigits.equals(iDigits))
            return true;
        return false;
    }

    // Compare date strings by digits only
    private boolean dateMatches(String dbDate, String inputDob) {
        if (dbDate == null || inputDob == null)
            return false;
        String dbNorm = dbDate.replaceAll("\\D", "");
        String inNorm = inputDob.replaceAll("\\D", "");
        return dbNorm.equals(inNorm);
    }
    ...
    public Optional<ThiSinhWithNguyenVongResponse> getThiSinhWithNguyenVongByCccd(String cccd) {
        ...
        // Bước 2: Lấy danh sách nguyên vọng theo CCCD (dùng CCCD thực tế từ DB)
        List<NguyenVong> nguyenVongList = nguyenVongRepository.findByCccd(thiSinh.getCccd());
    ...
    private ThiSinhWithNguyenVongResponse buildThiSinhWithNguyenVongResponse(
        ...
        List<ThiSinhWithNguyenVongResponse.NguyenVongInfo> nguyenVongInfoList = nguyenVongList.stream()
