...
/**
 * Dialog cho import danh sách thí sinh từ file Excel
 */
public class ImportNVExcelDialog extends JDialog {
    private File selectedFile;
    private boolean confirmed = false;
    private final JLabel fileLabel = new JLabel("Chưa chọn file");
    private final JProgressBar progressBar = new JProgressBar();

    public ImportNVExcelDialog(Frame owner) {
        super(owner, "Import Danh Sách Nguyện Vọng", true);
        setSize(500, 250);
		...
		if(currentUser.getIdRoleValue() == 3){
			// Hệ thống tự động cập nhật giá trị của cột CCCD với tài khoản  học sinh
			columnsInfo = "Cột dữ liệu: Mã Ngành, Thứ tự NV, Điểm THXT, Điểm UTQĐ, Điểm Cộng, Điểm Xét Tuyển, Kết Quả, Keys, Phương thức, Mã THM<br/>";
		}else{
			columnsInfo = "Cột dữ liệu: <b style='color:red;'>CCCD (Bắt buộc)</b>, Mã Ngành, Thứ tự NV, Điểm THXT, Điểm UTQĐ, Điểm Cộng, Điểm Xét Tuyển, Kết Quả, Keys, Phương thức, Mã THM";
		}
		
        // Info panel
		JLabel infoLabel = new JLabel("<html><b>Import danh sách nguyện vọng từ file Excel</b><br/>" +
